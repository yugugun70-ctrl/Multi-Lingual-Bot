...
import { getConfigStatus, saveConfig, loadConfig } from "../lib/config";
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .card{background:#1a1a2e;border:1px solid #2a2a4a;border-radius:16px;padding:36px;width:100%;max-width:540px;box-shadow:0 8px 40px rgba(0,0,0,.5)}
  ...
  .section-title{font-size:.8rem;font-weight:600;color:#7c82ff;text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px}
</style>
</head>
<body>
<div class="card">
  <h1>⚙️ EditAI — Setup Bot</h1>
  <p class="sub">Isi API key di bawah untuk mengaktifkan semua fitur bot</p>
  ...
  <form method="POST" action="/api/setup">
    ...
    <label>Token Bot Telegram <span class="required">*</span></label>
    <input type="password" name="TELEGRAM_BOT_TOKEN"
      ...
      autocomplete="off"/>
    <div class="hint">Dari @BotFather · Wajib diisi</div>

    <label>User ID Admin <span class="required">*</span></label>
    <input type="text" name="ADMIN_ID"
      placeholder="${status.ADMIN_ID ? fieldVal("ADMIN_ID") : "1234567890"}"
      value="${fieldVal("ADMIN_ID")}"
      autocomplete="off"/>
    <div class="hint">ID Telegram kamu sebagai admin bot · Cek via @userinfobot</div>

    <hr class="sep"/>
    <div class="section-title">🧠 NVIDIA AI (Chat + Text-to-Image)</div>

    <label>NVIDIA API Key <span class="required">*</span></label>
    <input type="password" name="NVIDIA_API_KEY"
      ...
      autocomplete="off"/>
    <div class="hint">Dari build.nvidia.com · Diperlukan untuk fitur AI chat & generate foto</div>

    <hr class="sep"/>
    <div class="section-title">🎬 Kling AI (Video Generation — Opsional)</div>

    <label>Kling Access Key</label>
    <input type="password" name="KLING_ACCESS_KEY"
      placeholder="${status.KLING_ACCESS_KEY ? "Sudah diset — kosongkan jika tidak ingin ubah" : "kling-access-..."}"
      autocomplete="off"/>

    <label>Kling Secret Key</label>
    <input type="password" name="KLING_SECRET_KEY"
      placeholder="${status.KLING_SECRET_KEY ? "Sudah diset — kosongkan jika tidak ingin ubah" : "kling-secret-..."}"
      autocomplete="off"/>
    <div class="hint">Dari platform.klingai.com · Opsional — tanpa ini video pakai FFmpeg fallback</div>

    <hr class="sep"/>
    <div class="section-title">🖼️ Remove.bg (Opsional)</div>

    <label>Remove.bg API Key</label>
    <input type="password" name="REMOVE_BG_API_KEY"
      placeholder="${status.REMOVE_BG_API_KEY ? "Sudah diset — kosongkan jika tidak ingin ubah" : "removebg-..."}"
      autocomplete="off"/>
    <div class="hint">Dari remove.bg · Opsional — tanpa ini pakai engine lokal gratis</div>
...
router.post("/setup", (req, res) => {
  try {
    ...
    res.send(
      renderPage(
        "✅ Konfigurasi berhasil disimpan! Restart server agar bot token baru aktif.",
        "success"
      )
    );
  } catch (e: any) {
